...
\begin{enumerate}
    \item \Gls{pixeliu}: evaluates overlap between predicted and ground-truth at the pixel level. It is defined as
    ...
    \item \Gls{lineiu}: evaluates overlap at the line level using the same IoU formula:
    \[ \text{Line IU} = \frac{TP}{TP + FP + FN} \]
    Here, TP is the number of mtched lines, FP are spurious lines, and FN are missed lines. A predicted line matches ground truth if both pixel-level precision and recall exceed 75\%.

    \item \Gls{dr}: proportion of matched ground-truth lines:
    \[ \text{DR} = \frac{M}{N_1} \]
    ...
    \item \Gls{ra}: proportion of matched predicted lines:
    \[ \text{RA} = \frac{M}{N_2} \]
    where \( N_2 \) is the number of predicted lines.

    \item \Gls{fm}: harmonic mean of DR and RA:
